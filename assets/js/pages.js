...
(function ($) {
    ...
    var app = angular.module('News', []).directive('menulistPostRepeatDirective', function () {
    ...
    ipNews = function ($scope, $location) {
        //init
        ...
        var showPages = function () {
            ...
            if ($scope.activeMenu.type == 'list') { // list view
                ...
                if (!gridContainer.data('gateway')) {
                    gridContainer.data('gateway', ip.baseUrl + '?aa=KbNews.pagesGridGateway&parentId=' + $scope.activeMenu.id);
        ...
        var setDefaultPositionForNextTime = function (alias, position, isPageSelected) {
            ...
            var data = {
                aa: 'KbNews.setDefaultPagePosition',
        ...
        var addMenu = function (title, type) {
            ...
            $.ajax({
                ...
                context: this,
                success: function (response) {
                    window.location = ip.baseUrl + '?aa=KbNews.index#/hash=&language=' + $scope.activeLanguage.code;
                    location.reload();
                },
                error: function (response) {
                    if (ip.developmentEnvironment || ip.debugMode) {
